...
export function normalizePropList(propList) {
...
export function getSelectedPropValues(propList, selectedProp) {
...
export function getSelectedProperties(propList, selectedProp) {
  const list = []
  ;(propList || []).forEach((prop) => {
    if (selectedProp[prop.k_id] && selectedProp[prop.k_id].length > 0) {
      const v = [];
      prop.v.forEach((it) => {
        if (selectedProp[prop.k_id].includes(it.id)) {
          v.push({ ...it });
        }
      });
      list.push({
