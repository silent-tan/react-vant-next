...
```jsx
import { Divider, Space, Typography } from "@react-vant-next/ui";
/**
 * title: 分隔符
 */
import React from "react";
...
export default () => {
  return (
    <Space direction="vertical">
      {Array.from({ length: 2 }).fill(null).map((_, index) => (
...
function Child({ children }) {
  return <div style={{ padding: 15, border: "1px solid #eee", borderRadius: 4 }}>{children}</div>;
}
