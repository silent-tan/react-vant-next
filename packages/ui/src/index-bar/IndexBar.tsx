...
import type { IndexBarInstance, IndexBarProps } from "./PropsType";
import { useEventListener, useMount, useRefs, useScrollParent, useTouch } from "@react-vant-next/hooks";
...
function IndexBar({ ref, ...p }: IndexBarProps & { ref?: React.RefObject<IndexBarInstance | null> }) {
      ...
      .map((child: ReactElement) => {
        if (child.type?.[COMPONENT_TYPE_KEY] === INDEX_ANCHORE_KEY) {
          return React.cloneElement<any>(child, {
            ref: setRefs((child.props as any).index),
          });
        }
        // TODO FIXME
        if ((child.props as any)?.children) {
          const deepMap = handleMapChildren((child.props as any)?.children);
  ...
  return (
    <IndexBarContext value={{ zIndex, highlightColor, sticky }}>
      <div
        ref={root}
