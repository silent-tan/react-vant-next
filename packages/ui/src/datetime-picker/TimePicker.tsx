import type { DateTimePickerInstance, TimePickerProps } from "./PropsType";

import { useUpdateEffect } from "@react-vant-next/hooks";

import { mergeProps, padZero, range } from "@react-vant-next/utils";
import React, { useMemo, useState } from "react";

import Picker from "../picker";
import { times } from "./utils";

function TimePicker({ ref, ...p }: TimePickerProps & { ref?: React.RefObject<DateTimePickerInstance | null> }) {
    ...
    () =>
      ranges.map(({ type, range: rangeArr }) => {
        let values = times(rangeArr[1] - rangeArr[0] + 1, index =>
          padZero(rangeArr[0] + index)) as string[];
