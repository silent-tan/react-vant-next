...
```ts
import type { DropdownItemOption, DropdownMenuDirection, DropdownMenuInstance } from "@react-vant-next/ui";
```

`DropdownMenuInstance` 是组件实例的类型，用法如下：

```ts
import type { DropdownMenuInstance } from "@react-vant-next/ui";
