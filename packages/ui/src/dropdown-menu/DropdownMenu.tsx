...
import { useClickAway, useEventListener, useMergedState, useRefs, useScrollParent, useUpdateEffect } from "@react-vant-next/hooks";
import { createNamespace, getRect, isDef, mergeProps } from "@react-vant-next/utils";
...
function DropdownMenu({ ref, ...p }: DropdownMenuProps & { ref?: React.RefObject<DropdownMenuInstance | null> }) {
  ...
  const showPopupIndexRef = useRef<number>(void 0);
  const root = useRef<HTMLDivElement>(void 0);
  const barRef = useRef<HTMLDivElement>(void 0);
