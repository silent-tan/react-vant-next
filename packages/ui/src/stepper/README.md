...
```js
import { Stepper } from "@react-vant-next/ui";
```
...
```ts
import type { StepperInstance, StepperTheme } from "@react-vant-next/ui";
```

`StepperInstance` 是组件实例的类型，用法如下：

```ts
import type { StepperInstance } from "@react-vant-next/ui";
