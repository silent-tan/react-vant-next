...
import type { StepperInstance, StepperProps } from "./PropsType";
import { usePropsValue, useRefState } from "@react-vant-next/hooks";
import {
  addUnit,
  bound,
  createNamespace,
...
function Stepper({ ref, ...p }: StepperProps & { ref?: React.RefObject<StepperInstance | null> }) {
  ...
  const format = (v: number) => {
    ...
    if (props.decimalLength !== undefined) {
      target = Number.parseFloat(target.toFixed(props.decimalLength));
    }
    return target;
  };
