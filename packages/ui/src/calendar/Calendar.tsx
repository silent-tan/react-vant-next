import type { PickerPopupActions } from "../picker/PropsType";
import type { CalendarDayItem, CalendarInstance, CalendarProps } from "./PropsType";
import { usePropsValue, useRefs, useSetState, useUpdateEffect } from "@react-vant-next/hooks";
import { createNamespace, getRect, getScrollTop, mergeProps, pick, raf } from "@react-vant-next/utils";
...
function Calendar({ ref, className, style, ...p }: CalendarProps & { ref?: React.RefObject<CalendarInstance | null> }) {
  ...
  const bodyRef = useRef<HTMLDivElement>(void 0);
  const bodyHeightRef = useRef<number>(0);

  const [value, setValue] = React.useState(() => getInitialDate(
  ...
  const buttonDisabled = useMemo(() => {
    if (state.currentDate) {
      if (props.type === "range") {
        return !(state.currentDate as Date[])[0] || !(state.currentDate as Date[])[1];
      }
      if (props.type === "multiple") {
        return !(state.currentDate as Date[]).length;
      }
    }
