...
import type { CircleProps, CircleStartPosition } from "./PropsType";
import { useMergedState } from "@react-vant-next/hooks";
import { cancelRaf, createNamespace, getSizeStyle, isObject, mergeProps, raf } from "@react-vant-next/utils";
...
const Circle: React.FC<CircleProps> = (p) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const crate = format(Number.parseFloat(rate.toFixed(1)));
