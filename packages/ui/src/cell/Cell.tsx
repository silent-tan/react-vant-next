...
import { Arrow, ArrowDown, ArrowLeft, ArrowUp } from "@react-vant-next/icons";
import { createNamespace, isDef } from "@react-vant-next/utils";
  ...
  const renderLeftIcon = () => {
    if (props.icon) {
      return React.cloneElement(props.icon as React.ReactElement<any>, {
        className: clsx((props.icon as React.ReactElement<any>)?.props?.className ?? "", bem("left-icon")),
      });
    }
    return null;
  };
