...
import type { TabsClickTabEventParams } from "../tabs/PropsType";
import type {
  CascaderOption,
  CascaderProps,
  CascaderTab,
...
import { useDebounceEffect, useMemoizedFn, usePropsValue, useUpdateEffect } from "@react-vant-next/hooks";
import { Cross, Success } from "@react-vant-next/icons";
import { createNamespace, extend, isObject, mergeProps } from "@react-vant-next/utils";
...
const Cascader: React.FC<CascaderProps> = (props) => {
  ...
  const renderCloseIcon = () => {
    if (!props.closeable)
      return null;
    if (props.closeIcon) {
      return React.cloneElement(props.closeIcon as React.ReactElement<any>, {
...
function CascaderPopup({ ref, ...p }: PopupCascaderProps & { ref?: React.RefObject<PickerPopupActions | null> }) {
  ...
  return (
    <>
      <Popup
        ...
        closeOnClickOverlay
        onClickOverlay={() => {
          if (!popupProps?.closeOnClickOverlay)
            return;
          setVisible(false);
        }}
        {...popupProps}
